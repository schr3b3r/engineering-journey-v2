...
## Current State
Milestone M8 completed — First-pass Notability Signal implemented in `notability.py` (`NotabilityEngine`, `NotabilitySignal`). Computes eventfulness scores using volume vs. personal baseline math (mean, std dev, z-scores, volume ratio) and category flags (`volume_surge`, `high_activity`, `quiet_period`, `first_activity`, `focus_switch`, `streak`). Stores scores in `value` of `NumericAnnotation` custom type records with period start `recorded_at`, filterable tags, and detailed baseline comparison JSON payloads in `note`. Full pytest test suite passing (50 tests across `tests/test_notability.py`, `tests/test_summarization.py`, `tests/test_rollups.py`, `tests/test_backfill.py`, `tests/test_raw_ingestion.py`, `tests/test_github_spike.py`, `tests/test_checkpoint.py`).
