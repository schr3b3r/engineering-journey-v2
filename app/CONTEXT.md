...
## Current State
Milestone M6 completed — "Activity Rollup" layer implemented in `rollups.py` (`ActivityRollup`, `RollupEngine`). Computes, persists, and queries rollups for all 5 period types (`day`, `week`, `month`, `quarter`, `year`) using `DurationAnnotation` base type with native `{start_time, end_time}` ranges in `recorded_at`. Performs deterministic hand-rolled numeric activity aggregation with real provenance chains (`sources`) linking Year -> Quarter -> Month -> Week/Day -> Raw activity items. Full pytest test suite passing (39 tests across `tests/test_rollups.py`, `tests/test_backfill.py`, `tests/test_raw_ingestion.py`, `tests/test_github_spike.py`, `tests/test_checkpoint.py`). Verified against live Fulcra API.
