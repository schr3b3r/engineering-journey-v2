...
## Current State
Milestone M7 completed — Harness-side Rollup Summarization implemented in `summarization.py` (`RollupSummarizer`, `build_summarization_prompt`, `format_rollup_summary_handoff`). Packages activity rollups into structured task prompts and input handoffs for the running agent, and performs deterministic write-back of summary text into the rollup's `note` JSON payload on Fulcra while preserving all native duration bounds, tags, and sources. Zero bundled LLM provider dependencies. Full pytest test suite passing (42 tests across `tests/test_summarization.py`, `tests/test_rollups.py`, `tests/test_backfill.py`, `tests/test_raw_ingestion.py`, `tests/test_github_spike.py`, `tests/test_checkpoint.py`).
