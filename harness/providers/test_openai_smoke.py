"""
Standalone smoke test for the OpenAI provider adapter.
...
you build anything on top of it. Run directly:
    ...
    .venv/bin/python harness/providers/test_openai_smoke.py

Requires OPENAI_API_KEY to be set -- this adapter is API-key-only, see
openai_provider.py's module docstring for why there is no OAuth path here.
"""
...
from openai_provider import call_model  # noqa: E402  (import after load_dotenv on purpose)
...
def main():
    ...
    print("text:", repr(resp.text))
    assert resp.text and "42" in resp.text
